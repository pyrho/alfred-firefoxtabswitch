 ...
 async function main() {
     ...
     switch (cmd) {
         case 'get':
             ...
             const titleMatchesAllFilters = e => filters.every(f => e.title.toLowerCase().indexOf(f) >= 0);
             const urlMatchesAllFilters = e => filters.every(f => e.subtitle.toLowerCase().indexOf(f) >= 0);

             const filterItemsIfNeeded = () => filters ?
                 formattedElements.filter(e => titleMatchesAllFilters(e) || urlMatchesAllFilters(e)) :
                 formattedElements;
